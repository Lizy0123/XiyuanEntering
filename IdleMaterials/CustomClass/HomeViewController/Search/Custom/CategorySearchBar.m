...
-(void)layoutSubviews
{
    ...
    [searchField setFrame:CGRectMake(83,4.8,self.frame.size.width-95,22)];
    searchField.tintColor= kColorMain;
    //
...
-(UIButton*)categoryBtn{
    if (!_categoryBtn) {
        ...
        _categoryBtn.frame=CGRectMake(5, 0, 70, 31);
        ...
        [_categoryBtn setTitleColor:kColorMain forState:UIControlStateNormal];
        [_categoryBtn setTitle:@"产品名称" forState:UIControlStateNormal];
    }
    return _categoryBtn;
}
...
-(UIButton*)iconBtn{
    if (!_iconBtn) {
        _iconBtn=[[UIButton alloc] initWithFrame:CGRectMake(75, 11, 8, 8)];
...
-(void)layoutSubviews
{
    ...
    UITextField *searchField = [[[[[self subviews] firstObject] subviews] filteredArrayUsingPredicate:finalPredicate] lastObject];
    searchField.textAlignment = NSTextAlignmentLeft;
//    [searchField setFrame:CGRectMake(-CGRectGetWidth(self.frame)/2 + 50, 4.8, CGRectGetWidth(self.frame), 22)];
    [searchField setFrame:CGRectMake(0, 4.8, CGRectGetWidth(self.frame), 22)];
