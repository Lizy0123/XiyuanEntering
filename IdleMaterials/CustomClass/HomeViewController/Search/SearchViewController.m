...
//                searchBar.searchTextPositionAdjustment=UIOffsetMake(10,0);
    ...
    [_mySearchBar patchWithCategoryWithSelectBlock:^{
        if ([KxMenu isShowingInView:[UIApplication sharedApplication].keyWindow]) {
            ...
            CGRect senderFrame = CGRectMake(weakSelf.searchView.frame.origin.x+70, 64, 0, 0);
    ...
    if (!_searchDisplayVC) {
        _searchDisplayVC = ({
            ...
            [searchVC.searchBar setPlaceholder:@"搜索"];
...
- (void)menuItemClicked:(KxMenuItem *)item{
    ...
    NSString *showStr=([[_statusList objectAtIndex:_selectIndex] length]>4)?[[_statusList objectAtIndex:_selectIndex] substringToIndex:[[_statusList objectAtIndex:_selectIndex] length]-2]:[_statusList objectAtIndex:_selectIndex];
